...
public class updateTeacher_record extends JFrame implements ActionListener {
    ...
    updateTeacher_record(String sm) {
        ...
        l13 = new JLabel("New Teacher Details");
     //   l14.setFont(f);
    ...
    void loadStudentDetails(String rollno) {
        try {
            ...
            String query = "select * from new_faculty where employ_id='" + rollno + "'";
            ResultSet rs = obj.stm.executeQuery(query);
            if (rs.next()) {
                t10.setText(rs.getString("employ_id"));
    ...
    public void actionPerformed(ActionEvent ev) {
        if (ev.getSource() == bt1) {
            ...
            try {
                connection obj = new connection();
                String query = "update new_faculty set name='" + name + "', father_name='" + fathername + "', age=" + age + ", dob='" + dob + "', address='" + address + "', phone='" + phone + "', email='" + email + "', classx=" + classX + ", classxii=" + classXII + ", department='" + branch + "', course='" + course + "' where employ_id='" + rollno + "'";
                obj.stm.executeUpdate(query);
                JOptionPane.showMessageDialog(null, "Data updated successfully");
                setVisible(false);
                new update_teacher().setVisible(false);
            } catch (Exception ex) {
                ex.printStackTrace();
            }
        }
